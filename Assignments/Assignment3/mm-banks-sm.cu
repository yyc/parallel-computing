...
// SHARED MEMORY KERNEL HERE
__global__ void banks_kernel(matrix a, matrix b, matrix result, int size)
{
	// Initialize shared memory
	__shared__ float aMat[BLOCKSIZE][BLOCKSIZE];
	__shared__ float bMat[BLOCKSIZE][BLOCKSIZE];
...
void work()
{
	...
	dim3 block(BLOCKSIZE, BLOCKSIZE);			// a block of CUDA threads
	...
	sm_kernel<<<grid, block>>>(a, bt, result1, size);
	cudaDeviceSynchronize();

	after = wall_clock_time();
	time1 = ((float)(after - before))/1000000000;
	fprintf(stderr, "SM on GPU took %1.2f seconds\n", time1);
	// was there any error?
    rc = cudaGetLastError();
    if (rc != cudaSuccess)
	...
	dim3 bblock(BLOCKSIZE, BLOCKSIZE);			// a block of 32 x 32 CUDA threads
	dim = paddedSize / BLOCKSIZE;
	dim3 bgrid(dim , dim );	// a grid of CUDA thread blocks
	before = wall_clock_time();
	init_matrix_zero(bt);
	transpose_kernel<<<transgrid, transblock>>>(b, bt, size);
	cudaDeviceSynchronize();

	banks_kernel<<<bgrid, bblock>>>(a, bt, result2, size);
	cudaDeviceSynchronize();

	after = wall_clock_time();
	time2 = ((float)(after - before))/1000000000;
	fprintf(stderr, "Banks on GPU took %1.2f seconds\n", time2);
	// was there any error?
    rc = cudaGetLastError();
    if (rc != cudaSuccess)
            printf("Last CUDA error %s\n", cudaGetErrorString(rc));
...
int main(int argc, char ** argv)
{
	...
	fprintf(stderr,"SM/SM-Banks multiplication of size %d\n", size);
